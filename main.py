...
def create_instagram_story_image(sp, top_artist_id, top_artists_data, top_tracks_data):
    ...
    output_path = f"historial_semanal_{int(datetime.datetime.now().timestamp())}.jpg"
...
def main():
    ...
    try:
        ...
        caption = f"🎵 ¡Tu resumen REAL semanal está listo! (Semana del {now.split()[0]})\nEscuchaste {len(data)} canciones en total."
